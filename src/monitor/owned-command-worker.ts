// Shared lifecycle for monitor-owned commands. Prep and full preflight supply
// their own arguments, result contract, persistence and survivor namespace;
// process containment, close confirmation and shutdown retain one implementation.
...
export interface OwnedCommandJob<TResult> {
  taskId: string;
  pid: number;
  startedAt: string;
...
export interface OwnedCommandShutdownOptions {
  gracefulTimeoutMs?: number;
  forceTimeoutMs?: number;
}

export interface OwnedCommandShutdownResult {
  requested: string[];
  exited: string[];
  escalated: string[];
  timedOut: string[];
...
export interface OwnedCommandShutdownSurvivor {
  version: 1;
  taskId: string;
  pid: number;
  startedAt: string;
...
export interface OwnedCommandWorkerRuntime {
...
export interface OwnedCommandWorkerOptions<TResult> {
  keyManager?: KeyManager;
  label: "Prep" | "Preflight";
  survivorNamespace: "prep-shutdown-survivors" | "preflight-shutdown-survivors";
  outputLimit: number;
  outputUnit: "bytes" | "characters";
  commandArgs: (taskId: string, jobId: string) => string[];
  parseResult: (value: unknown, jobId: string, sanitize: (value: string) => string) => TResult;
  store: {
    read: (taskId: string) => OwnedCommandJob<TResult> | undefined;
    write: (job: OwnedCommandJob<TResult>) => void;
  };
  onTerminal?: (job: OwnedCommandJob<TResult>) => void;
}

const DIAGNOSTIC_OUTPUT_LIMIT = 64 * 1024;
const OWNED_COMMAND_SURVIVOR_VERSION = 1;

export class OwnedCommandWorker<TResult> {
  private jobs = new Map<string, OwnedCommandJob<TResult>>();
  ...
  private shutdownSurvivors = new Map<string, OwnedCommandShutdownSurvivor>();
  ...
  private shutdownPromise: Promise<OwnedCommandShutdownResult> | null = null;

  constructor(
    private readonly projectRoot: string,
    private readonly quackBin: string,
    private readonly runtime: OwnedCommandWorkerRuntime,
    private readonly options: OwnedCommandWorkerOptions<TResult>,
  ) {
    this.refreshShutdownSurvivors();
  }

  private shutdownSurvivorDir(): string {
    return path.join(this.projectRoot, ".quack", "logs", this.options.survivorNamespace);
  }
  ...
  private parseShutdownSurvivor(markerPath: string): OwnedCommandShutdownSurvivor | undefined {
    try {
      const parsed = JSON.parse(fs.readFileSync(markerPath, "utf-8")) as unknown;
      if (
        typeof parsed !== "object" ||
        parsed === null ||
        (parsed as { version?: unknown }).version !== OWNED_COMMAND_SURVIVOR_VERSION ||
      ...
      return {
        ...(parsed as Omit<OwnedCommandShutdownSurvivor, "strategy">),
        strategy:
          (parsed as { strategy?: OwnedCommandShutdownSurvivor["strategy"] }).strategy ??
  ...
  private refreshShutdownSurvivors(): void {
    ...
    const survivors = new Map<string, OwnedCommandShutdownSurvivor>();
  ...
  private recordShutdownAttempt(
    taskId: string,
    child: ChildProcess,
    strategy: OwnedCommandShutdownSurvivor["strategy"],
  ): OwnedCommandShutdownSurvivor {
    ...
    if (existing) {
      ...
      throw new Error(
        `${this.options.label} process-tree shutdown for ${taskId} is already unconfirmed; refusing to re-signal stored PID ${existing.pid}`,
      );
    }

    const markerPath = this.shutdownSurvivorPath(taskId);
    if (fs.existsSync(`${markerPath}.lock`)) {
      throw new Error(`${this.options.label} shutdown reconciliation is locked for ${taskId}`);
    }
    const marker: OwnedCommandShutdownSurvivor = {
      version: OWNED_COMMAND_SURVIVOR_VERSION,
      taskId,
      pid: child.pid,
    ...
    try {
      ...
      if (winner) {
        throw new Error(
          `${this.options.label} process-tree shutdown for ${taskId} is already unconfirmed; refusing to re-signal stored PID ${winner.pid}`,
        );
      }
      throw error;
    }
  ...
  private clearShutdownSurvivor(marker: OwnedCommandShutdownSurvivor): boolean {
  ...
  getShutdownSurvivors(): OwnedCommandShutdownSurvivor[] {
    this.refreshShutdownSurvivors();
   ...
   */
  start(taskId: string, jobId: string = randomUUID()): OwnedCommandJob<TResult> {
    if (this.terminalDrainStarted) {
      throw new Error(`${this.options.label} admission is closed because the monitor is shutting down.`);
    }
    if (this.shutdownInProgress) {
      throw new Error(`${this.options.label} worker is shutting down`);
    }
    this.refreshShutdownSurvivors();
    if (this.shutdownSurvivors.size > 0 || this.unreadableSurvivorMarkers.size > 0) {
      throw new Error(
        `Cannot start ${this.options.label.toLowerCase()} while unresolved Windows shutdown survivor evidence or POSIX process-group evidence remains`,
      );
    }
    // Prevent double-prep
    const existing = this.getActiveJob(taskId);
    if (existing) {
      throw new Error(`Task ${taskId} ${this.options.label.toLowerCase()} is already running (pid ${existing.pid})`);
    }

    // Each operation supplies fixed CLI arguments and an attempt identity.
    const args = this.options.commandArgs(taskId, jobId);
    ...
    const job: OwnedCommandJob<TResult> = {
      taskId,
      jobId,
    ...
    let stdoutSize = 0;
    let stderrSize = 0;
    const stdoutCounter = new StringDecoder("utf8");
    const stderrCounter = new StringDecoder("utf8");
    ...
    const sanitize = (value: string): string => {
      ...
      return this.options.outputUnit === "characters"
        ? safe.slice(0, DIAGNOSTIC_OUTPUT_LIMIT)
        : new StringDecoder("utf8").write(Buffer.from(safe).subarray(0, DIAGNOSTIC_OUTPUT_LIMIT));
    };
    this.stopRequestedTasks.delete(taskId);
    this.confirmedWindowsTreeKills.delete(taskId);

    child.stdout?.on("data", (data: Buffer) => {
      stdoutSize += this.options.outputUnit === "bytes" ? data.length : stdoutCounter.write(data).length;
      stdoutTruncated ||= stdoutSize > this.options.outputLimit;
      if (!stdoutTruncated) stdoutChunks.push(data);
    });

    child.stderr?.on("data", (data: Buffer) => {
      stderrSize += this.options.outputUnit === "bytes" ? data.length : stderrCounter.write(data).length;
      stderrTruncated ||= stderrSize > DIAGNOSTIC_OUTPUT_LIMIT;
      if (!stderrTruncated) stderrChunks.push(data);
    });
    ...
    child.on("error", (err) => {
      ...
      if (this.terminationUnconfirmed.has(taskId)) {
        job.error = job.error ?? `${this.options.label} process-tree termination remains unconfirmed`;
        return;
      }
      if (this.processes.get(taskId) === child) this.processes.delete(taskId);
      if (this.launches.get(taskId) === launch) {
    ...
    const finalize = (): void => {
      ...
      job.diagnostics = {
        stdout: stdoutTruncated
          ? `[${this.options.label.toLowerCase()} stdout exceeded output limit; content omitted]`
          : sanitize(stdout),
        stderr: stderrTruncated
          ? `[${this.options.label.toLowerCase()} stderr exceeded output limit; content omitted]`
          : sanitize(stderr),
        stdoutTruncated,
        stderrTruncated,
      };
      ...
      if (this.stopRequestedTasks.has(taskId) || this.stopping.has(taskId)) {
        job.error = `${this.options.label} stopped by operator`;
      } else if (spawnError) {
        job.error = spawnError;
      } else if (exitCode !== 0 || exitSignal) {
        job.error =
          job.diagnostics.stderr ||
          `${this.options.label} child exited with code ${exitCode}, signal ${exitSignal ?? "none"}`;
      } else if (stdoutTruncated) {
        job.error = `${this.options.label} output exceeded the bounded result size; refusing an incomplete result`;
      } else {
        try {
          job.result = this.options.parseResult(JSON.parse(stdout.trim()) as unknown, jobId, sanitize);
          job.status = "completed";
          delete job.error;
        } catch (error) {
          job.error = sanitize(
            `Invalid ${this.options.label.toLowerCase()} output: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      this.stopping.delete(taskId);
      try {
        this.options.store.write(job);
      } catch (error) {
        job.persistenceError = sanitize(
          `${this.options.label} diagnostic persistence failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      ...
      try {
        ...
        console.error(`${this.options.label} terminal observer failed: ${sanitize(String(error))}`);
      }
    };
  ...
  stop(taskId: string): boolean {
    ...
    const job = this.jobs.get(taskId);
    if (job) job.error = `${this.options.label} stop requested`;
    if (alreadyExited) {
      this.stopping.delete(taskId);
      if (job) job.error = `${this.options.label} stopped by operator`;
      if (this.processes.get(taskId) === child) this.processes.delete(taskId);
   ...
   */
  getJob(taskId: string): OwnedCommandJob<TResult> | undefined {
    return this.jobs.get(taskId) ?? this.options.store.read(taskId);
  }

  /**
   * Get active (running) job for a task.
   */
  getActiveJob(taskId: string): OwnedCommandJob<TResult> | undefined {
    const job = this.jobs.get(taskId);
   ...
   * Get all active jobs.
   */
  getActiveJobs(): OwnedCommandJob<TResult>[] {
  ...
  private async performShutdownAll(options: OwnedCommandShutdownOptions): Promise<OwnedCommandShutdownResult> {
   ...
   */
  async shutdownAll(options: OwnedCommandShutdownOptions = {}): Promise<OwnedCommandShutdownResult> {
